...
const resize = async (input,output) =>{
    try{
        let transform = await sharp()
                        .resize({ width: 80,height:80,fit:"contain",background:{ r: 255, g: 255, b: 255, alpha:0.1}})
                        .on('info', function(fileInfo) {
                            console.log("Resizing done, file not saved");
                        });
                        let inStream = await fs.createReadStream(`img5/${input}.jpeg`);
                        let outStream = await fs.createWriteStream(`img7/${output}.jpeg`, {flags: "w"});
